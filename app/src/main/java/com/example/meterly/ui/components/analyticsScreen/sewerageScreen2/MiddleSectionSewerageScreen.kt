...
@Composable
fun MiddleSectionSewerageScreen2(
    payment: Payment? = null,
    allPayments: List<Payment> = emptyList(),
    startAxisSewerage: () -> Axis<Axis.Position.Vertical.Start>?,
    bottomAxisSewerage: () -> Axis<Axis.Position.Horizontal.Bottom>?,
    modelProducerSewerage: CartesianChartModelProducer,
    roundAmounts: Boolean = false
) {
    val startAxis = if (allPayments.size > 1) rememberMonthStartAxis() else startAxisSewerage()
    val bottomAxis = if (allPayments.size > 1) rememberMonthBottomAxis(allPayments) else bottomAxisSewerage()
            ...
            Column(
                ...
                if (allPayments.isEmpty()) {
                    ...
                    CartesianChartHost(
                        ...
                        modelProducer = modelProducerSewerage,
                        modifier = Modifier
                            .fillMaxWidth()
                            .height(200.dp)
                    )
    ...
    CostsSewerage(payment = payment, roundAmounts = roundAmounts)
}

@Composable
fun CostsSewerage(payment: Payment? = null, roundAmounts: Boolean = false){
